...
namespace TVStation.Controllers
{
    ...
    public class ProgramFrameWeekController : ControllerBase
    {
        private readonly IProgramFrameWeekRepository _repository;
        private readonly UserManager<User> _userManager;
        public ProgramFrameWeekController(IProgramFrameWeekRepository repository, UserManager<User> userManager)
        {
        ...
        [HttpGet]
        [Authorize]
        public IActionResult GetAllPaging([FromQuery] ProgramFrameWeekQuery query)
        {
            try
            {
                var res = _repository.GetAllPaging(query);
                return Ok(res.Map<PlanListDTO<ProgramFrameWeek>, PlanListDTO<PlanDTO>>());
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetById([FromRoute] Guid id)
        {
            try
            {
                ...
                return Ok(res.Map<ProgramFrameWeek, ProgramFrameWeekDTO>());
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message);
        ...
        [HttpPost]
        [Authorize]
        public IActionResult Create([FromBody] ProgramFrameWeekDTO dto)
        {
            try
            {
                ...
                var data = dto.Map<ProgramFrameWeekDTO, ProgramFrameWeek>();
                data.Creator = user;
                ...
                return Ok(result.Map<ProgramFrameWeek, ProgramFrameWeekDTO>());
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message);
        ...
        [HttpPut("{id}")]
        [Authorize]
        public IActionResult Update([FromRoute] Guid id, [FromBody] ProgramFrameWeekDTO dto)
        {
            try
            {
                if (!ModelState.IsValid) return BadRequest(ModelState);
        ...
        public IActionResult GetByStatus([FromBody] string status)
        {
            try
            {
                ...
                return Ok(res.Select(i => i.Map<ProgramFrameWeek, ProgramFrameWeekDTO>()));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message);
